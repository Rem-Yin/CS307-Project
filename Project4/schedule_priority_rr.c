...
void add(char *name, int priority, int burst){
	...
	struct task *newTask = malloc(sizeof(struct task));			// important to allocate memory space for variables!!!
	newTask->name = (char *)malloc(sizeof(char)*(1 + strlen(name)));// DONNOT be silly again!!! 
...
int pick_NextTask(){
	...
	while(tmp != NULL){
		//Find the thread with the highest priority closest to the end of the queue
